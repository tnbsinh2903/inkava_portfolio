...
export const OurTeam: React.FC<OurTeamProps> = ({ team = [], bgColor }) => {
  ...
  return (
    <section 
      id="team" 
      className="w-full py-24 scroll-mt-20 overflow-hidden transition-colors duration-500"
    ...
    >
      <div className="w-full">
        ...
        <div className="relative w-full overflow-hidden py-4 select-none">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-16 md:w-32 bg-gradient-to-r from-inherit to-transparent z-10 pointer-events-none" />
          <div className="absolute right-0 top-0 bottom-0 w-16 md:w-32 bg-gradient-to-l from-inherit to-transparent z-10 pointer-events-none" />

          {/* Marquee Track - scrolls from left to right */}
          <div className="flex w-[200%] md:w-[150%] xl:w-[120%]">
            <div className="flex gap-6 shrink-0 min-w-full justify-around items-center animate-marquee-ltr hover-pause cursor-pointer py-2">
              {marqueeItems.map((member, idx) => (
                <div 
                  key={`${member.id}-${idx}`}
                  className="w-64 sm:w-72 bg-white rounded-3xl p-6 border border-slate-200/40 shadow-sm hover:shadow-md hover:border-amber-500/20 transition-all duration-300 flex flex-col items-center text-center group shrink-0 relative overflow-hidden"
                >
                  {/* Subtle top background highlight */}
                  <div className="absolute top-0 left-0 right-0 h-2 bg-gradient-to-r from-amber-500 to-amber-600 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                  ...
                  <div className="mt-4 pt-4 border-t border-slate-100 w-full space-y-2">
                    ...
                    {member.email && (
                      <div className="flex items-center justify-center gap-1.5 text-xs text-gray-400 truncate">
                        ...
                        <span className="font-mono text-[10.5px] lowercase">{member.email}</span>
                      </div>
                    )}
                  </div>
                </div>
